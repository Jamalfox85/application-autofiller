...
After a purchase the extension keeps Pro in the local entitlement cache and in `pendingProfilePlan` for offline UX. The server plan is set only by `POST /api/v1/billing/plan` with `X-API-Key`, `Authorization: Bearer <supabase access token>`, and `{ "plan": "pro" }`. A success body is `{ "success": true, "data": { "plan": "pro" } }`. The client does not update `profiles.plan`. `POST /api/v1/resumes/generate` and `POST /api/v1/ats/analyze` send the same Bearer token. A 403 whose error code is `plan_required` (including a nested `error.code`) opens the Pro upgrade UI. Dashboard SKUs on `gofillr` are `pro_monthly` ($5.99/mo) and `pro_annual` ($49/yr).
